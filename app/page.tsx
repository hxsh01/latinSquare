...
export default function Home() {
  ...
  const difficultyDescription = useMemo(
    () =>
      ({
        easy: "The highlighted cell can be reached with short, direct deductions.",
        medium: "The highlighted cell requires a few linked deductions.",
        hard: "The highlighted cell requires a longer chain of deductions.",
      })[difficulty],
    [difficulty],
  );
  function submit() {
      ...
      <div className="mx-auto max-w-4xl">
        <header className="mb-6 flex flex-col gap-5 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
              Find the symbol that belongs in the highlighted cell. The number
              of clues stays similar across levels; difficulty comes from the
              reasoning required.
            </p>
          </div>
        </header>
        <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-soft sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <button
              ...
              onClick={generate}
              disabled={generating}
              className="h-11 rounded-xl bg-blue-600 px-5 text-sm font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60 sm:min-w-44"
            >
              {generating
                ? "Generating…"
                : puzzle
        ...
        {puzzle && (
          <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-soft sm:p-6">
            ...
            {result && (
              <div
                className={`mt-6 rounded-2xl border p-4 ${result === "correct" ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}
              >
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p
                      className={`font-bold ${result === "correct" ? "text-green-800" : "text-red-800"}`}
                    >
